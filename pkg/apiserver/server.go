...
func (s *Server) handleSetDir(w http.ResponseWriter, r *http.Request) {
	...
	// Ensure saveDir ends with a slash
	saveDir := body.SaveDir
	if saveDir != "" && saveDir[len(saveDir)-1] != '/' {
		saveDir += "/"
	}

	if err := s.setSaveDirFn(saveDir); err != nil {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]string{"error": err.Error()}) //nolint:errcheck
		return
	...
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]string{"saveDir": saveDir}) //nolint:errcheck
}
...
func (s *Server) handleSetDest(w http.ResponseWriter, r *http.Request) {
	...
	// Update the cached destIP for /api/status
	s.mu.Lock()
	s.destIP = body.DestIP
	s.mu.Unlock()
